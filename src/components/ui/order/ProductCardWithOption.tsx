...
const ProductCardWithOption: React.FC<Props> = ({ item, children ,confirmation}) => {
    console.log("im cart product",item)
    return (
        <View style={styles.card}>
            <Image source={require('@/assets/images/Mask Group.png')} style={styles.image} />
            <View style={styles.cardContainer}>
                <View style={styles.textContainer}>
                    <Text style={styles.name}>{confirmation && `${item?.quantity}x`} {item.name}</Text>
                    ...
                    item?.options?.map((option)=>(
                        ...
                        <ListOption name={option?.modifierName} />
                        {
                            option?.modifireOptions?.map((modifiers)=>(
                                <ListModifier name={modifiers?.modifierOptionName} price={modifiers?.modifierOptionPrice} />
